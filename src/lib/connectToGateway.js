...
import dumpWriters from "./dumpWriters";
...
function connectToGateway(archive, updateSyncStatus, updateConnecting) {
  ...
  const port = process.env.NODE_ENV === "production" ? "" : ":3300";
  ...
  function connectWebsocket() {
    ...
    function goOnlineNow() {
      if (!connected && archiveStream.destroyed) {
        console.log(
          "Browser went online - Restarting replication on websocket"
        );
        connectWebsocket();
      }
    }
